...
export class LoginComponent implements OnInit {
  ...
  usuariosins: UsuariosIns[] = [];
  
  registro: UsuariosIns = {
    nombres: '',
    apellidos: '',
    email: '',
    password: '',
  ...
  async registrarse(){
    if(this.registro.nombres === ''){
      ...
      if(res){
        ...
        const path = 'UsuariosIns';
        const uid = res.user.uid;
        this.registro.uid = uid;
        await this.firestoreService.createDoc(this.registro, path, uid);
        console.log('Guardando con exito',res);
        this.interaction.closeLoading();
        this.interaction.presentToast('Guardando con exito');
      }
    }
  }
  ...
  getUserInfo(uid: string){
    ...
    const path = 'UsuariosIns';
    this.suscriberUserInfo = this.firestoreService.getDoc<UsuariosIns>(path, this.uid).subscribe(res =>{
      this.registro = res;
    });
  }
